...
class Ui_MainWidget(object):
    def setupUi(self, mainWindow, localServer, argv=None):
        ...
        adminLogin = self.menubar.addMenu(' &登录')

        # saveLogAnalyticsAction = QtGui.QAction('&商品管理', mainWindow)
        ...
        managerUser = self.menubar.addMenu('&用户管理')

        managerCate = self.menubar.addMenu('&分类管理')

        managerGoods = self.menubar.addMenu('&商品管理')

        managerOrder = self.menubar.addMenu('&订单管理')

        managerBanner = self.menubar.addMenu('&公告管理')

        managerOther = self.menubar.addMenu(' &其他')

        otherExpressAction = QtGui.QAction('快递查询', mainWindow)
        otherBillAction = QtGui.QAction('账单统计', mainWindow)
